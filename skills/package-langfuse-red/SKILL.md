...
out of the installed package. The ClickHouse templates are this package's own, derived
...
./red create             # converge for real; the gates run inside it
./red rehearse           # restore both stores from backup, boot the pinned image, drill
./red describe           # every host's last monitor result, over SSH
./red delete             # guarded by compute-prevent-destroy; removes nothing in R2
```
...
`./red rehearse` then proves recovery: fresh sets, restore both stores,
