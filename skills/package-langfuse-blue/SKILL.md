...
out of the installed `package-neon-blue` distribution. The ClickHouse templates
are this package's own, derived from
[`getcolors/clickhouse`](https://github.com/getcolors/clickhouse).

## Install the launcher

```sh
npx skills add getcolors/langfuse
cp .agents/skills/package-langfuse-blue/blue ./blue
chmod +x blue
```
...
./blue rehearse           # restore both stores from backup, boot the pinned image, drill
./blue describe           # every host's last monitor result, over SSH
./blue delete             # guarded by compute-prevent-destroy; removes nothing in R2
```
...
`./blue rehearse` then proves recovery: fresh sets, restore both stores,
