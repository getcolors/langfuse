...
curl -sf "http://127.0.0.1:<{ clickhouse-http-port }>/ping" >/dev/null || problems+=("clickhouse does not answer /ping")
...
r=$(cq "SELECT count() FROM clusterAllReplicas('<{ clickhouse-cluster-name }>', system.one)"); [ "${r:-0}" = "3" ] || problems+=("cluster sees ${r:-0} replicas, not 3")
...
if [ -f /etc/colors/backup-r2.env ]; then
  ...
  [ "$age" -le <{ langfuse-clickhouse-backup-max-age-hours }> ] || problems+=("newest completed clickhouse backup is ${age}h old")
fi
