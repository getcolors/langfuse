...
export const tool = "langfuse-storage";
export const managed = (opts: Opts): boolean => opts["langfuse-storage-managed"] === true;
export const directory = (opts: Opts): string => stageDir(opts, tool, {defaultProfile: "langfuse"});
...
export async function ownershipPreflight(opts: Opts): Promise<void> {
  ...
  for (const [role,key] of [["neon","neon-r2-bucket"],["data","langfuse-s3-bucket"],["backup","langfuse-backup-r2-bucket"]]) {
...
export async function storageStep(opts: Opts): Promise<Opts> {
  ...
  try {
    ...
    return await tofu.tofuWithSpec(opts,documents,{dir:directory(opts),env:awsEnv(opts),outputKey:"langfuse/storage-credentials"});
...
export function credentialEnv(opts: Opts): Record<string,string> {
  const credentials = opts["langfuse/storage-credentials"]?.credentials ?? {};
  const environment: Record<string,string> = {ANSIBLE_HOST_KEY_CHECKING:"False"};
  for (const [role,prefix] of [["neon","NEON_R2"],["data","LANGFUSE_STORAGE_R2"],["backup","LANGFUSE_BACKUP_R2"]]) {
...
export async function readCredentials(opts: Opts): Promise<Opts> {
  ...
  try {
    ...
    const result = {...opts,"langfuse/storage-credentials":await tofu.outputs(directory(opts),awsEnv(opts))};
