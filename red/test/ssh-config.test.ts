...
describe("ssh-config", () => {
  test("the bare profile plus one alias per machine", () => {
    ...
    expect(sc.machineAlias(renamed, { node_id:"clickhouse-1", role: "clickhouse", index: 1, name: "custom-clickhouse-1" } as topology.Host))
      .toBe("langfuse-test-clickhouse-1");
    expect(sc.machineAlias(renamed, { node_id:"app-0", role: "app", index: null, name: "custom-app" } as topology.Host))
      .toBe("langfuse-test-app-0");
  ...
  test("a foreign stanza for any alias is detected", () => {
    ...
    expect(sc.foreignStanzaLine(lines, "langfuse-test-app-0", "langfuse-test")).toBeUndefined();
    ...
    expect(sc.foreignStanzaLine(own, "langfuse-test-redis-0", "langfuse-test")).toBeUndefined();
  });
