...
role="langfuse"
db="langfuse"
bucket="langfuse-storage-fixture"
prefix="langfuse-fixture/neon"
...
if [ "$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null)" != "langfuse-optout-fixture" ]; then
  ready=$(mktemp); printf '%s' "langfuse-optout-fixture" > "$ready"
  rclone copyto "$ready" "r2:$bucket/$prefix/.colors-ready"
  rm -f "$ready"
  back=$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null || true)
  if [ "$back" != "langfuse-optout-fixture" ]; then
    echo "neon-smoke: ready marker read back as '$back', expected 'langfuse-optout-fixture'" >&2
    exit 1
  fi
