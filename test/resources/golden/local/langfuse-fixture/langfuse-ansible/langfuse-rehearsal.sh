...
scratch_db=$(printf '%s' "$live_db" | sed -E 's#/langfuse$#/langfuse_restore_check#')
umask 077
...
services:
  web:
    image: docker.langfuse.com/langfuse/langfuse:4.27.0@sha256:c9e2cab8469a5d7353e86a3252b02c52ac94ef31288ce2639ee01aabf5e4222b
    restart: "no"
    env_file:
...
if [ "$st" = "200" ] && printf '%s' "$body" | grep -q '"fixture-project"'; then
