...
if [ "$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null)" != "langfuse-fixture" ]; then
  ready=$(mktemp); printf '%s' "langfuse-fixture" > "$ready"
  rclone copyto "$ready" "r2:$bucket/$prefix/.colors-ready"
  rm -f "$ready"
  back=$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null || true)
  if [ "$back" != "langfuse-fixture" ]; then
    echo "neon-smoke: ready marker read back as '$back', expected 'langfuse-fixture'" >&2
    exit 1
  fi
