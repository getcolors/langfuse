...
cors=$(curl -sS --max-time 20 -o /dev/null -w '%{http_code}' -X OPTIONS -H "Origin: https://langfuse.fixture.example" -H 'Access-Control-Request-Method: GET' "https://fixture.r2.cloudflarestorage.com/$STORE_BUCKET/" 2>/dev/null || echo 000)
...
[ "${cors_hdr:-0}" -gt 0 ] && pass "M3 the storage bucket answers CORS preflight for https://langfuse.fixture.example" \
  || warn "M3 no CORS on $STORE_BUCKET for https://langfuse.fixture.example (preflight $cors): media renders in the UI only after the operator adds the CORS rule from the README"
