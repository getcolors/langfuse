...
if rows=$(wait_trace "$T" <{ langfuse-smoke-timeout-seconds }>); then
...
[ "${cors_hdr:-0}" -gt 0 ] && pass "M3 the storage bucket answers CORS preflight for https://<{ langfuse-host }>" \
  || warn "M3 no CORS on $STORE_BUCKET for https://<{ langfuse-host }>: media renders in the UI only after the operator adds the CORS rule from the README"
...
echo "== P: throughput =="
N=<{ langfuse-smoke-traces }>; sent=0; spans=""; i=0
while [ "$i" -lt "$N" ]; do
