...
# (`test/resources/resource-addresses-keygen.txt` and `-optout.txt`): every
# `resource "…" "…"` line and every `count` / `for_each` line of the rendered
# compute stage, in file order — the block addresses and the count expressions
# a plan is computed from. `langfuse-vultr` is live, so a resource that moves
# is a machine that is destroyed; the manifests are never regenerated by
# `--accept`, and a change to them is a deliberate, hand-made decision. The
# opt-out manifest lacks `vultr_ssh_key.machine` by design. This proves the
# configuration, not the live state: it is not a `tofu state list`.
#
...
for variant in colors optout; do
  ...
  # anything: a renamed or re-counted resource fails here in both modes.
  mode=keygen; [[ $variant == optout ]] && mode=optout
  manifest="$root/test/resources/resource-addresses-$mode.txt"
  if ! diff -u "$manifest" <(grep -E '^resource "|^\s+(count|for_each)\s*=' "$infra"); then
