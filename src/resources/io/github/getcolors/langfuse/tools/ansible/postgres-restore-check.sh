...
psql_admin() { env -i PATH=/usr/bin:/bin PGPASSWORD="$admin_pw" psql -w "postgresql://cloud_admin@127.0.0.1:<{ neon-compute-port }>/postgres?connect_timeout=10" -v ON_ERROR_STOP=1 -tAc "$1"; }
psql_scratch() { env -i PATH=/usr/bin:/bin PGPASSWORD="$role_pw" psql -w "postgresql://<{ neon-role }>@127.0.0.1:<{ neon-compute-port }>/$SCRATCH?connect_timeout=10" -tAc "$1"; }

psql_admin "DROP DATABASE IF EXISTS $SCRATCH WITH (FORCE)" >/dev/null
psql_admin "CREATE DATABASE $SCRATCH OWNER <{ neon-role }>" >/dev/null
...
[ "${foreign:-1}" = "0" ] || { echo "postgres-restore-check: $foreign tables are not owned by <{ neon-role }>" >&2; exit 1; }
projects=$(psql_scratch "select count(*) from projects" 2>/dev/null || echo "n/a")
echo "postgres-restore-check: restored $SET into $SCRATCH ($tables tables, $projects projects), all owned by <{ neon-role }>"
