...
from . import ssh, ssh_config, tools, validate, topology
...
async def start_step(original, env=None):
    ...
    async def after(opts, _env, context):
        if context['real'] and context['event'] in STATE_EVENTS:
            result = await read_deployment(opts, environment, None, topology.requirements(opts))
...
def wire_fn(step: str, run_opts: dict):
    ...
    if event == "delete":
        return {
            ...
            "langfuse/infrastructure": (tools.infrastructure_step,),
        }.get(step)
    if event == "rehearse":
        return {
...
def create_workflow():
    wf = workflow(start="langfuse/start", wire_fn=wire_fn, next_fn=lambda step, successors, opts: [] if opts.get("langfuse/already-destroyed") or failed(opts) else [(successor, opts) for successor in successors or []])
