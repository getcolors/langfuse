...
tool = "langfuse-storage"


def managed(opts):
    return opts.get("langfuse-storage-managed") is True
...
async def ownership_preflight(opts):
    ...
    for role, key in [("neon", "neon-r2-bucket"), ("data", "langfuse-s3-bucket"), ("backup", "langfuse-backup-r2-bucket")]:
...
async def storage_step(opts):
    ...
    try:
        ...
        return await tofu.tofu_with_spec(opts, documents, dir=directory(opts), env=aws_env(opts), output_key="langfuse/storage-credentials")
    except Exception:
...
def credential_env(opts):
    credentials = opts.get("langfuse/storage-credentials", {}).get("credentials", {})
    environment = {"ANSIBLE_HOST_KEY_CHECKING": "False"}
    for role, prefix in [("neon", "NEON_R2"), ("data", "LANGFUSE_STORAGE_R2"), ("backup", "LANGFUSE_BACKUP_R2")]:
...
async def read_credentials(opts):
    ...
    try:
        ...
        result = {**opts, "langfuse/storage-credentials": await tofu.outputs(directory(opts), aws_env(opts))}
