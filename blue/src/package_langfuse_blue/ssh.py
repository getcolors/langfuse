...
def rendered_only(opts: dict) -> bool:
    ...
    standard holds both to the same rule — neither may read, create, or
    require anything under `~/.ssh`, and both must render byte-identically
    whether or not the keypair exists. A dry-run is a create that touches
    nothing, so testing the event alone would let it reach the real key path."""
...
def with_machine_key(opts: dict) -> dict:
    ...
    if not build:
        return opts
    profile = opts.get("profile") or "langfuse"
