...
[ "$age" -le <{ langfuse-postgres-backup-max-age-hours }> ] || problems+=("newest completed postgres backup is ${age}h old")
mage=$(newest_set_age_hours "backup:$BACKUP_BUCKET/$PROFILE/media-runs")
[ "$mage" -le <{ langfuse-media-backup-max-age-hours }> ] || problems+=("newest completed media run is ${mage}h old")
