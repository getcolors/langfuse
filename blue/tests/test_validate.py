...
def test_reports_every_problem_at_once():
    assert len(errs({"neon-pg-version": 12, "redis-port": None, "vultr-vpc-subnet": "invalid"})) >= 3
...
def test_compute_policy_errors_come_from_the_library():
    assert errs({'provider-compute': 'unsupported'})
    assert errs({'vultr-ssh-sources': []})
    assert errs({'vultr-ssh-sources': ['1.2.3.4']})
    assert errs({'vultr-vpc-subnet': '10.50.0.1/24'})
    assert errs({'vultr-plan-app': None})
    assert errs({'vultr-http-sources': ['1.2.3.0/24'], 'cloudflare-proxied': False}) == []
